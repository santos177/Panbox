...
public class AdminSQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        for (int i = 0; i < Clientes.length; i++) {
            db.execSQL("insert into clientes (nombre_cliente, precio_unitario, saldo_anterior, total, saldo, tipo, cobro) VALUES ('"+ Clientes[i] +"','"+ tipo_default +"','"+ td +"','"+ tipo_default +"','"+ tipo_default +"','"+ tipo_default +"','"+ tipo_default +"')");
        }
  ...
  @Override
    public void onUpgrade(SQLiteDatabase db, int version1, int version2) {
        ...
        db.execSQL("create table clientes (nombre_cliente text, precio_unitario integer, total_pan float, saldo_anterior integer, saldo integer, cobro integer)");
